...
func test_stage4_direction_construction() -> void:
	...
	assert_eq(d.start.coords, Vector2(0, 0), "Start should match")
	assert_eq(d.end.coords, Vector2(1, 0), "End should match")
...
func test_stage4_ray_construction() -> void:
	var d := Direction.from_coords(Vector2(0, 0), Vector2(1, 0))
	var r := Ray.from_coords(Vector2(10, 20), d)
	assert_eq(r.origin.coords, Vector2(10, 20), "Ray origin should match")
	assert_eq(r.direction, d, "Ray direction should match")

func test_stage4_ray_preserves_direction_reference() -> void:
	var d := Direction.from_coords(Vector2(0, 0), Vector2(1, 0))
	var r1 := Ray.from_coords(Vector2(10, 20), d)
	var r2 := Ray.from_coords(Vector2(30, 40), d)
