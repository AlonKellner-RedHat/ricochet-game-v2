...
func test_stage7_collinear_produces_line() -> void:
	var seg := Segment.new(Vector2(0, 0), Vector2(10, 0), Vector2(5, 0), true)
	assert_true(seg.get_carrier().is_line(), "Line-by-construction should produce a line carrier")
...
func test_stage7_via_inf_produces_line() -> void:
	...
	assert_true(seg.is_line(), "via=INF should be line by construction")
	assert_true(seg.get_carrier().is_line(), "via=INF should produce a line carrier")

func test_stage7_is_line_flag() -> void:
	var line_seg := Segment.new(Vector2(0, 0), Vector2(10, 0), Vector2(5, 0), true)
	assert_true(line_seg.is_line(), "Segment created with is_line=true should report is_line")
	var arc_seg := Segment.new(Vector2(200, 100), Vector2(200, 300), Vector2(300, 200))
	assert_false(arc_seg.is_line(), "Segment without is_line flag should not report is_line")

func test_stage7_side_determination_line() -> void:
	var seg := Segment.new(Vector2(100, 400), Vector2(100, 200), Vector2(100, 300), true)
